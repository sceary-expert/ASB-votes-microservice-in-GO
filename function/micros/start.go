...
import (
	...
	coreSetting "example.com/core/config"
	"example.com/core/data/mongodb"
	coreUtils "example.com/core/utils"
)
...
var secretKeys = []string{mongoHostSecretKey, mongoDatabaseSecretKey, payloadSecretKey, publicKeySecretKey}
...
func loadSecretsFromFile(newCoreConfig *config.Configuration) {
	...
	if filesConfig[basePath+mongoHostSecretKey] != nil {
		...
		mongoDBHost := "mongodb+srv://asb:asbdesigngo@cluster0.ninbdmn.mongodb.net/?retryWrites=true&w=majority"
		newCoreConfig.MongoDBHost = &mongoDBHost
		log.Printf("[INFO]: MongoDB host information loaded from env." + mongoDBHost)
	}

	if filesConfig[basePath+mongoDatabaseSecretKey] != nil {
		// mongoDB := string(filesConfig[basePath+mongoDatabaseSecretKey])
		mongoDB := "telar-social-media"
		newCoreConfig.Database = &mongoDB
		log.Printf("[INFO]: Database name information loaded from env." + mongoDB)
	}
